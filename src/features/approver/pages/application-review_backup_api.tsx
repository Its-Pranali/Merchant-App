...
export function ApplicationReviewPage() {
  ...
  const { data: application, isLoading } = useQuery({
    ...
    const appId = id;
    const [applications, setApplication] = useState<any>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!appId) return; // don't call API if no ID yet

    const fetchApplication = async () => {
      try {
        setLoading(true);
        const res = await axios.get(          
          `http://192.168.0.143:8086/api/approver/applications/reviewApplication/${appId}`
        );
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold break-words">{applications.businessName}</h1>
              <p className="text-muted-foreground">Application Review</p>
            </div>
          </div>
          <StatusChip status={applications.status} />
        </div>
        ...
        <Tabs defaultValue="details" className="space-y-6">
          ...
          <TabsContent value="details" className="space-y-6">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium break-words">{applications.businessName}</p>
                  </div>
                  {applications.tradeName && (
                    <div>
                      <Label className="text-muted-foreground">Trade Name</Label>
                      <p className="font-medium break-words">{applications.tradeName}</p>
                    </div>
                  )}
                  <div>
                    <Label className="text-muted-foreground">PAN</Label>
                    <p className="font-mono font-medium">{applications.pan}</p>
                  </div>
                  {applications.gstin && (
                    <div>
                      <Label className="text-muted-foreground">GSTIN</Label>
                      <p className="font-mono font-medium">{applications.gstin}</p>
                    </div>
                  )}
                </div>
              </CardContent>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium break-words">{applications.contactName}</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Phone</Label>
                    <p className="font-medium">{applications.phone}</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Email</Label>
                    <p className="font-medium break-all">{applications.email}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <p className="font-medium break-words">{applications.address1}</p>
                  {applications.address2 && <p>{applications.address2}</p>}
                  <p className="break-words">{applications.city}, {applications.state} - {applications.pincode}</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {format(new Date(applications.createdAt), 'PPp')}
                        {applications.agentName && ` by ${applications.agentName}`}
                      </p>
                    </div>
                  </div>
                  ...
                  {applications.status !== 'DRAFT' && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {format(new Date(applications.updatedAt), 'PPp')}
                        </p>
                      </div>
                    </div>
                  )}

                  {applications.discrepancyItems && applications.discrepancyItems.length > 0 && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {applications.discrepancyItems.map((item, index) => (
